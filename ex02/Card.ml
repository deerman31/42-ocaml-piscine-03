...
let allhelper color = List.map (fun x -> newCard x color) all
let allSpades = allhelper Spade
let allHearts = allhelper Heart
let allDiamonds = allhelper Diamond
let allClubs = allhelper Club
...
let toString t = toString (getValue t) ^ toString (getColor t)

let toStringVerbose t =
  "Card("
  ^ toStringVerbose (getValue t)
  ^ ", "
  ^ toStringVerbose (getColor t)
  ^ ")"

let compare t1 t2 =
  let rec loop acc v2 = if acc = v2 then -1 else loop (next acc) v2 in
